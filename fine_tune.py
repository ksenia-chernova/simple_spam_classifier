...
new_ds = load_dataset("benzlokzik/russian-spam-fork")['train'].select(range(100_000))

new_ds = new_ds.rename_column("label", "is_spam")
new_ds = new_ds.cast_column("is_spam", Value("int64"))
new_ds = new_ds.map(lambda x: {"is_spam": int(x["is_spam"])})

ds = concatenate_datasets([old_ds, new_ds])
...
model.save_pretrained("./final_spam_model_v2.0")
tokenizer.save_pretrained("./final_spam_model_v2.0")